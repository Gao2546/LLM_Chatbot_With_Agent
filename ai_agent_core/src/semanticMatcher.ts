...
export interface Intent {
  type: 'definition' | 'howto' | 'troubleshooting' | 'comparison' | 'recommendation' | 'explanation' | 'listing' | 'general';
...
export interface HybridSearchResult {
  ...
  confidenceScore: number;  // 0-1 (weighted combination)
...
export function detectIntent(question: string): Intent {
  ...
  let maxScore = 0;
  let detectedIntent: Intent['type'] = 'general';
 ...
 */
export function calculateKeywordScore(text1: string, text2: string): KeywordMatch {
 ...
 * - 720 days old: 0.0625
 */
export function calculateFreshnessScore(createdAt: Date, halfLifeDays: number = 180): number {
  const now = new Date();
...
/**
 * Hybrid semantic search combining multiple signals
 * 
 * Scoring weights:
 * - Vector Similarity: 55%
 * - Keyword Matching: 30%
 ...
 * @param vectorResults - Results from vector similarity search (with similarity score)
...
export interface HybridSearchOptions {
  ...
  minConfidence?: number;     // Default: 0.25 (filter threshold)
  intentMatching?: boolean;   // Default: true (use intent detection)
}

export function hybridSemanticSearch(
  query: string,
  vectorResults: any[],
...
export function filterByIntent(
  ...
  targetIntent: Intent['type'],
  minIntentConfidence: number = 0.5
): HybridSearchResult[] {
 ...
 */
export function explainMatch(result: HybridSearchResult): string {
  const parts = [];
...
export function getConfidenceColor(score: number): string {
  if (score >= 0.70) return '#10b981'; // green
