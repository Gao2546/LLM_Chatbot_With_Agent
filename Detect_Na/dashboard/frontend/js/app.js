...
function openLiveFailDetail(index) {
    ...
    info.innerHTML = `
        ...
        <div style="display:grid; grid-template-columns: repeat(6, 1fr); gap:12px; margin-bottom:16px;">
            <div style="background:var(--bg-card); border-radius:8px; padding:14px; border:1px solid var(--border-color); text-align:center;">
                <div style="font-size:12px; color:var(--text-muted); font-weight:600; margin-bottom:6px; text-transform:uppercase;">Machine</div>
                <div style="font-size:16px; font-weight:700; color:var(--text-primary);">${machineName}</div>
            </div>
            <div style="background:var(--bg-card); border-radius:8px; padding:14px; border:1px solid var(--border-color); text-align:center;">
                <div style="font-size:12px; color:var(--text-muted); font-weight:600; margin-bottom:6px; text-transform:uppercase;">Status</div>
                <div style="font-size:16px; font-weight:700; color:${statusColor};">${statusLabel}</div>
            </div>
            <div style="background:var(--bg-card); border-radius:8px; padding:14px; border:1px solid var(--border-color); text-align:center;">
                <div style="font-size:12px; color:var(--text-muted); font-weight:600; margin-bottom:6px; text-transform:uppercase;">Error Code</div>
                <div style="font-size:16px; font-weight:700; color:var(--text-primary);">${event.error_id}</div>
            </div>
            <div style="background:var(--bg-card); border-radius:8px; padding:14px; border:1px solid var(--border-color); text-align:center;">
                <div style="font-size:12px; color:var(--text-muted); font-weight:600; margin-bottom:6px; text-transform:uppercase;">Severity</div>
                <div style="font-size:16px; font-weight:700; color:${statusColor};">${(event.severity || "unknown").toUpperCase()}</div>
            </div>
            <div style="background:var(--bg-card); border-radius:8px; padding:14px; border:1px solid var(--border-color); text-align:center;">
                <div style="font-size:12px; color:var(--text-muted); font-weight:600; margin-bottom:6px; text-transform:uppercase;">Fail Ratio</div>
                <div style="font-size:16px; font-weight:700; color:var(--text-primary);">${failPct}</div>
            </div>
            <div style="background:var(--bg-card); border-radius:8px; padding:14px; border:1px solid var(--border-color); text-align:center;">
                <div style="font-size:12px; color:var(--text-muted); font-weight:600; margin-bottom:6px; text-transform:uppercase;">Clusters</div>
                <div style="font-size:16px; font-weight:700; color:var(--text-primary);">${clusterCount}</div>
            </div>
        </div>
        
        <!-- Live Fail Detail Section -->
        <div style="background:var(--bg-card); border-radius:8px; padding:14px; border-left:4px solid ${statusColor};">
            <div style="font-weight:600; color:var(--text-primary); margin-bottom:12px; font-size:14px;">Live Fail Detail</div>
            <div style="font-size:13px; color:var(--text-secondary); line-height:1.8;">
                <div><strong>Description:</strong> ${event.description || '-'}</div>
                <div style="margin-top:8px;"><strong>Seen:</strong> <span style="color:var(--text-primary); font-weight:700;">${event.seen_count || 0}x</span></div>
                <div style="margin-top:4px;"><strong>First Detected:</strong> ${event.first_seen_at ? new Date(event.first_seen_at).toLocaleString() : '-'}</div>
                <div style="margin-top:4px;"><strong>Last Detected:</strong> ${event.last_seen_at ? new Date(event.last_seen_at).toLocaleString() : '-'}</div>
                <div style="margin-top:4px;"><strong>Detection Method:</strong> <code style="font-size:12px; background:var(--bg-primary); padding:2px 6px; border-radius:4px; display:inline-block;">${event.method || '-'}</code></div>
            </div>
        </div>
    `;
...
function openErrorDetail(index) {
    ...
    if (item.isLive) {
        ...
        // Info for live events - match reference design
        const severity = item.severity || item.sevClass || "unknown";
        const severityBg = severity === "high" || severity === "critical" ? "#cf222e" :
                          severity === "medium" || severity === "warning" ? "#bf8700" : "#0A8276";
        
        info.innerHTML = `
            <!-- Header Cards -->
            <div style="display:grid; grid-template-columns: repeat(4, 1fr); gap:12px; margin-bottom:16px;">
                <div style="background:var(--bg-card); border:1px solid var(--border-color); border-radius:6px; padding:12px; text-align:center;">
                    <div style="font-size:11px; color:var(--text-muted); text-transform:uppercase; font-weight:600; margin-bottom:6px;">Machine</div>
                    <div style="font-size:14px; font-weight:700; color:var(--text-primary);">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:middle; margin-right:4px;"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"/><line x1="2" y1="17" x2="22" y2="17"/></svg>
                        ${item.machine}
                    </div>
                </div>
                <div style="background:var(--bg-card); border:1px solid var(--border-color); border-radius:6px; padding:12px; text-align:center;">
                    <div style="font-size:11px; color:var(--text-muted); text-transform:uppercase; font-weight:600; margin-bottom:6px;">Category</div>
                    <div style="font-size:13px; font-weight:700; color:var(--text-primary);">${item.category}</div>
                </div>
                <div style="background:var(--bg-card); border:1px solid var(--border-color); border-radius:6px; padding:12px; text-align:center;">
                    <div style="font-size:11px; color:var(--text-muted); text-transform:uppercase; font-weight:600; margin-bottom:6px;">Error Code</div>
                    <div style="font-size:16px; font-weight:700; color:var(--text-primary); font-family:monospace;">${item.errorCode || '-'}</div>
                </div>
                <div style="background:var(--bg-card); border:1px solid var(--border-color); border-radius:6px; padding:12px; text-align:center;">
                    <div style="font-size:11px; color:var(--text-muted); text-transform:uppercase; font-weight:600; margin-bottom:6px;">Severity</div>
                    <div style="display:inline-block; background:${severityBg}; color:#fff; padding:4px 8px; border-radius:4px; font-size:12px; font-weight:700;">${severity.toUpperCase()}</div>
                </div>
            </div>
            
            <!-- Red Alert Banner -->
            <div style="background:#cf222e; color:#fff; padding:12px 16px; border-radius:6px; margin-bottom:16px; display:flex; align-items:center; gap:8px;">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M12 1L2 6v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V6l-10-5z"/><path d="M10 17h4v2h-4zm0-4h4v2h-4zm0-4h4v2h-4z" fill="#cf222e" stroke="#fff"/></svg>
                <div style="font-size:14px; font-weight:700;">ERROR ${item.errorCode || 'UNKNOWN'}</div>
            </div>
            
            <!-- Main Description -->
            <div style="margin-bottom:12px;">
                <div style="font-size:18px; font-weight:700; color:var(--text-primary); margin-bottom:6px;">${item.description || 'Error detected'}</div>
                <div style="font-size:13px; color:var(--text-secondary);">
                    ${item.category} · Error detected at: <strong>${item.machine}</strong> · Seen: <strong>${item.seenCount || 0}x</strong>
                </div>
            </div>
            
            <!-- 3-Column Grid: Root Cause, Troubleshooting, Final Action -->
            <div style="display:grid; grid-template-columns: repeat(3, 1fr); gap:16px;">
                <!-- Root Cause -->
                <div style="background:#fef3f2; border-radius:6px; padding:16px; text-align:left;">
                    <div style="font-size:12px; font-weight:700; color:#6e5347; text-transform:uppercase; margin-bottom:12px; display:flex; align-items:center; gap:6px; letter-spacing:0.5px;">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
                        Root Cause
                    </div>
                    <ul style="margin:0; padding-left:18px; color:var(--text-secondary); font-size:13px; line-height:1.6;">
                        <li style="margin:0 0 6px 0;">Device not ready</li>
                        <li style="margin:0 0 6px 0;">Sensor blocked</li>
                    </ul>
                </div>
                
                <!-- Troubleshooting -->
                <div style="background:#f0f3f7; border-radius:6px; padding:16px; text-align:left;">
                    <div style="font-size:12px; font-weight:700; color:#3b5998; text-transform:uppercase; margin-bottom:12px; display:flex; align-items:center; gap:6px; letter-spacing:0.5px;">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 1 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg>
                        Troubleshooting
                    </div>
                    <ol style="margin:0; padding-left:20px; color:var(--text-secondary); font-size:13px; line-height:1.8;">
                        <li style="margin-bottom:8px;"><span style="display:inline-flex; align-items:center; justify-content:center; width:24px; height:24px; background:#3b5998; color:#fff; border-radius:50%; font-weight:700; font-size:12px; margin-right:6px;">1</span>Check if device is powered on</li>
                        <li style="margin-bottom:8px;"><span style="display:inline-flex; align-items:center; justify-content:center; width:24px; height:24px; background:#3b5998; color:#fff; border-radius:50%; font-weight:700; font-size:12px; margin-right:6px;">2</span>Clean sensor area</li>
                        <li style="margin-bottom:8px;"><span style="display:inline-flex; align-items:center; justify-content:center; width:24px; height:24px; background:#3b5998; color:#fff; border-radius:50%; font-weight:700; font-size:12px; margin-right:6px;">3</span>Restart the module</li>
                    </ol>
                </div>
                
                <!-- Final Action -->
                <div style="background:#ecfdf5; border-radius:6px; padding:16px; text-align:left;">
                    <div style="font-size:12px; font-weight:700; color:#155e3b; text-transform:uppercase; margin-bottom:12px; display:flex; align-items:center; gap:6px; letter-spacing:0.5px;">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>
                        Final Action
                    </div>
                    <div style="color:var(--text-secondary); font-size:13px; line-height:1.6;">
                        After fixing, press the machine reset button to resume normal operation.
                    </div>
                </div>
            </div>
        `;
    ...
    const errorCode = item.errorCode || firstCls.error_id || "-";
    const description = item.description || firstCls.desc || "-";
    const category = item.category || firstCls.cat || "-";
    const severity = item.severity || firstCls.sev || "unknown";
    const handler = item.handler || firstCls.handler || "Unknown";
    ...
    let extraData = null;
    if (firstCls.extra) {
        if (typeof firstCls.extra === 'string') {
            try {
                extraData = JSON.parse(firstCls.extra);
            } catch (e) {
                extraData = firstCls.extra;
            }
        } else {
            extraData = firstCls.extra;
        }
    ...
    // Extract downtime from extra field or direct field
    let downtime = item.downtime || "Not specified";
    let recovery = item.recovery || "";
    
    if (extraData && extraData.downtime) {
        downtime = extraData.downtime;
    } else if (firstCls.downtime) {
        downtime = firstCls.downtime;
    }
    
    if (extraData && extraData.recovery) {
        recovery = extraData.recovery;
    } else if (firstCls.recovery) {
        recovery = firstCls.recovery;
    }
    
    // Fetch error code details from database synchronously using async approach
    let causes = [];
    let steps = [];
    ...
    const fetchErrorDetails = async () => {
        try {
            const response = await fetch(`${API_BASE}/api/error-codes/${encodeURIComponent(errorCode)}`);
            const data = await response.json();
            
            if (data && data.extra) {
                let dbExtra = data.extra;
                if (typeof dbExtra === 'string') {
                    dbExtra = JSON.parse(dbExtra);
                }
                
                // Update downtime
                if (dbExtra.downtime) {
                    downtime = dbExtra.downtime;
                    const downtimeEl = info.querySelector('#downtimeValue');
                    if (downtimeEl) downtimeEl.textContent = downtime;
                }
                
                // Update causes
                if (dbExtra.cause && Array.isArray(dbExtra.cause) && dbExtra.cause.length > 0) {
                    causes = dbExtra.cause;
                    const causeList = info.querySelector('ul');
                    if (causeList) {
                        causeList.innerHTML = causes.map(c => `<li style="margin:0 0 6px 0;">${c}</li>`).join('');
                    }
                }
                
                // Update steps
                if (dbExtra.steps && Array.isArray(dbExtra.steps) && dbExtra.steps.length > 0) {
                    steps = dbExtra.steps;
                    const stepList = info.querySelector('ol');
                    if (stepList) {
                        stepList.innerHTML = steps.map((s, i) => `
                            <li style="margin-bottom:8px; display:flex; gap:10px;">
                                <span style="display:inline-flex; align-items:center; justify-content:center; width:24px; height:24px; background:#3b5998; color:#fff; border-radius:50%; font-weight:700; font-size:12px; flex-shrink:0;">${i + 1}</span>
                                <span style="margin-top:2px;">${s}</span>
                            </li>
                        `).join('');
                    }
                }
                ...
                // Update recovery
                if (dbExtra.recovery) {
                    recovery = dbExtra.recovery;
                }
            }
        } catch (e) {
            console.log("Could not fetch error code details:", e);
        }
    };
    
    // Parse causes and steps from classifications first (as fallback)
    if (cls.length > 0) {
        const c = cls[0];
        
        // Try to parse extra field if it's a JSON string
        let extraData = null;
        if (c.extra) {
            if (typeof c.extra === 'string') {
                try {
                    extraData = JSON.parse(c.extra);
                } catch (e) {
                    extraData = c.extra;
                }
            } else {
        ...
        // Extract cause from extra field
        if (extraData && extraData.cause && Array.isArray(extraData.cause)) {
            causes.push(...extraData.cause);
        } else if (c.causes && Array.isArray(c.causes)) {
            causes.push(...c.causes.slice(0, 2));
        }
        
        // Extract steps from extra field
        if (extraData && extraData.steps && Array.isArray(extraData.steps)) {
            steps.push(...extraData.steps);
        } else if (c.steps && Array.isArray(c.steps)) {
            steps.push(...c.steps.slice(0, 4));
        }
        
        // Extract recovery from extra field 
        if (extraData && extraData.recovery && !recovery) {
            recovery = extraData.recovery;
        }
    }
    
    // Add sample data if empty
    if (causes.length === 0 && category.toLowerCase().includes('cassette')) {
        causes.push("Device not inserted correctly", "Cassette sensor blocked");
    }
    if (steps.length === 0 && category.toLowerCase().includes('cassette')) {
        steps.push(
            "Check if cassette is inserted properly.",
            "Remove the cassette and insert it again.",
            "Clean the cassette sensor area.",
            "Restart the cassette loader module."
        );
    }
    
    // Call fetch in background to update with database data
    if (errorCode && errorCode !== "-") {
        fetchErrorDetails();
    }

    // Build reference design layout
    const severityBg = severity === 'high' || severity === 'critical' ? '#cf222e' :
                      severity === 'medium' || severity === 'warning' ? '#bf8700' : '#0A8276';
    ...
    info.innerHTML = `
        <div style="display:grid; grid-template-columns: repeat(4, 1fr); gap:12px; margin-bottom:16px;">
            ...
            <div style="background:var(--bg-card); border:1px solid var(--border-color); border-radius:6px; padding:12px; text-align:center;">
                <div style="font-size:11px; color:var(--text-muted); text-transform:uppercase; font-weight:600; margin-bottom:6px;">Category</div>
                <div style="font-size:13px; font-weight:700; color:var(--text-primary);">${category}</div>
            </div>
            <div style="background:var(--bg-card); border:1px solid var(--border-color); border-radius:6px; padding:12px; text-align:center;">
                <div style="font-size:11px; color:var(--text-muted); text-transform:uppercase; font-weight:600; margin-bottom:6px;">Error Code</div>
                <div style="font-size:16px; font-weight:700; color:var(--text-primary); font-family:monospace;">${errorCode}</div>
            </div>
            <div style="background:var(--bg-card); border:1px solid var(--border-color); border-radius:6px; padding:12px; text-align:center;">
                <div style="font-size:11px; color:var(--text-muted); text-transform:uppercase; font-weight:600; margin-bottom:6px;">Severity</div>
                <div style="display:inline-block; background:${severityBg}; color:#fff; padding:4px 8px; border-radius:4px; font-size:12px; font-weight:700;">${severity.toUpperCase()}</div>
        ...
        <!-- Main Description -->
        <div style="margin-bottom:12px;">
            <div style="font-size:18px; font-weight:700; color:var(--text-primary); margin-bottom:6px;">${description}</div>
            <div style="font-size:13px; color:var(--text-secondary);">
                ${category} · Error detected at: <strong>${item.machine}</strong> · Estimated Downtime: <strong id="downtimeValue">${downtime}</strong>
            </div>
        </div>
        ...
        <div style="display:grid; grid-template-columns: repeat(3, 1fr); gap:16px;">
            ...
            <div style="background:#fef3f2; border-radius:6px; padding:16px; text-align:left;">
                ...
                <ul style="margin:0; padding-left:18px; color:var(--text-secondary); font-size:13px; line-height:1.6;">
                    ${causes.map(c => `<li style="margin:0 0 6px 0;">${c}</li>`).join('')}
                </ul>
            </div>
            
            <!-- Troubleshooting -->
            <div style="background:#f0f3f7; border-radius:6px; padding:16px;">
                <div style="font-size:12px; font-weight:700; color:#3b5998; text-transform:uppercase; margin-bottom:12px; display:flex; align-items:center; gap:6px; letter-spacing:0.5px;">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 1 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg>
                    Troubleshooting
                </div>
                <ol style="margin:0; padding-left:0; color:var(--text-secondary); font-size:13px; line-height:1.8; list-style:none;">
                    ${steps.map((s, i) => `
                        <li style="margin-bottom:8px; display:flex; gap:10px;">
                            <span style="display:inline-flex; align-items:center; justify-content:center; width:24px; height:24px; background:#3b5998; color:#fff; border-radius:50%; font-weight:700; font-size:12px; flex-shrink:0;">${i + 1}</span>
                            <span style="margin-top:2px;">${s}</span>
            ...
            <div style="background:#ecfdf5; border-radius:6px; padding:16px;">
                ...
                <div style="color:var(--text-secondary); font-size:13px; line-height:1.6;">
                    After fixing the ${category.toLowerCase()} issue, press the machine reset button to resume operation.
                </div>
            </div>
        </div>
    `;
...
function openWaferDetail(index) {
    ...
    info.innerHTML = `
        ...
        <div style="display:grid; grid-template-columns: repeat(6, 1fr); gap:12px; margin-bottom:16px;">
            <div style="background:var(--bg-card); border-radius:8px; padding:14px; border:1px solid var(--border-color); text-align:center;">
                <div style="font-size:12px; color:var(--text-muted); font-weight:600; margin-bottom:6px; text-transform:uppercase;">Machine</div>
                <div style="font-size:16px; font-weight:700; color:var(--text-primary);">${item.machine}</div>
            </div>
            <div style="background:var(--bg-card); border-radius:8px; padding:14px; border:1px solid var(--border-color); text-align:center;">
                <div style="font-size:12px; color:var(--text-muted); font-weight:600; margin-bottom:6px; text-transform:uppercase;">Status</div>
                <div style="font-size:16px; font-weight:700; color:${statusColor};">${statusLabel}</div>
            ...
            <div style="background:var(--bg-card); border-radius:8px; padding:14px; border:1px solid var(--border-color); text-align:center;">
                <div style="font-size:12px; color:var(--text-muted); font-weight:600; margin-bottom:6px; text-transform:uppercase;">Location</div>
                <div style="font-size:16px; font-weight:700; color:var(--text-primary);">${locationLabel}</div>
            </div>
            <div style="background:var(--bg-card); border-radius:8px; padding:14px; border:1px solid var(--border-color); text-align:center;">
                <div style="font-size:12px; color:var(--text-muted); font-weight:600; margin-bottom:6px; text-transform:uppercase;">Largest Cluster</div>
                <div style="font-size:16px; font-weight:700; color:var(--text-primary);">${item.largest_cluster_ratio.toFixed(1)}%</div>
            </div>
            <div style="background:var(--bg-card); border-radius:8px; padding:14px; border:1px solid var(--border-color); text-align:center;">
                <div style="font-size:12px; color:var(--text-muted); font-weight:600; margin-bottom:6px; text-transform:uppercase;">Red Ratio</div>
                <div style="font-size:16px; font-weight:700; color:var(--text-primary);">${item.red_ratio.toFixed(1)}%</div>
            </div>
            <div style="background:var(--bg-card); border-radius:8px; padding:14px; border:1px solid var(--border-color); text-align:center;">
                <div style="font-size:12px; color:var(--text-muted); font-weight:600; margin-bottom:6px; text-transform:uppercase;">Total Clusters</div>
                <div style="font-size:16px; font-weight:700; color:var(--text-primary);">${item.total_clusters}</div>
            </div>
        </div>
    `;
