...
def index():
    ...
    try:
        ...
        print(f"[DEBUG] Index route - Frontend dir: {frontend_dir}")
        print(f"[DEBUG] Index route - Frontend exists: {os.path.isdir(frontend_dir)}")
        result = send_from_directory(frontend_dir, "index.html")
        print(f"[DEBUG] Index route - Successfully sent index.html")
        return result
    except Exception as e:
...
@app.route("/api/error-codes/<path:error_id>", methods=["GET"])
def api_error_codes_get(error_id):
    """Get a single error code by ID."""
    try:
        codes = db_helper.get_all_error_codes()
        for code in codes:
            if code["error_id"] == error_id:
                return jsonify(code)
