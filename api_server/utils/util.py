...
# ✅ เปลี่ยนเป็น model ที่ให้ 1024 dimensions (สำหรับ verified_answers table)
model = SentenceTransformer("sentence-transformers/all-MiniLM-L6-v2", device="cpu")
# Alternative models ที่ให้ 1024 dimensions:
# - sentence-transformers/all-mpnet-base-v2 (768 dims - ไม่ใช่)
# - sentence-transformers/paraphrase-multilingual-mpnet-base-v2 (768 dims - ไม่ใช่)
# - jinaai/jina-embeddings-v4 (1024 dims - ✅ recommended)
...
def encode_text_for_embedding(text: str, target_dimensions: int = 1024) -> list[float]:
    ...
    # --- FALLBACK: Use local SentenceTransformer ---
    print(f"Generating embedding using local SentenceTransformer model (target: {target_dimensions} dims).")
    embedding = model.encode(text, device='cpu', task='retrieval')
    
    # ตรวจสอบว่า embedding ไม่ว่าง
    if embedding is None or len(embedding) == 0:
        print(f"❌ ERROR: Model returned empty embedding!")
        raise ValueError("Model failed to generate embedding")
    
    # แปลง numpy array เป็น list
    embedding_list = embedding.tolist()
    
    # ตรวจสอบ dimensions ที่ได้
    actual_dims = len(embedding_list)
    print(f"Generated embedding: {actual_dims} dimensions")
    
    # ⚠️ Warning ถ้า dimensions ไม่ตรง
    if actual_dims != target_dimensions:
        print(f"⚠️ WARNING: Expected {target_dimensions} dimensions, got {actual_dims}")
        print(f"   Model used: {model.__class__.__name__}")
        print(f"   Make sure using correct model for verified_answers (should be 1024 dims)")
...
def process_pages_with_vlm(search_results: List[Dict[str, Any]], original_query: str, use_ollama: bool = False, ollama_model: str = "llava", deepinfra_model: str = "Qwen/Qwen2.5-VL-32B-Instruct") -> str:
    """
    ...
    4. Sends all images + query to a VLM (either Ollama or DeepInfra).
    5. Returns the VLM's text response.
    
    Args:
        search_results: List of search result dictionaries with page info.
        original_query: The user's original query.
        use_ollama: If True, use Ollama; if False, use DeepInfra.
        ollama_model: Ollama model name (default: 'llava').
        deepinfra_model: DeepInfra model name (default: 'Qwen/Qwen2.5-VL-32B-Instruct').
...
"""
    ...
    # Choose between Ollama and DeepInfra based on use_ollama flag
    if use_ollama:
        print(f"Using Ollama model: {ollama_model}")
        vlm_response = OllamaInference(
            prompt=prompt,
            system_prompt=system_prompt,
            image_bytes_list=image_bytes_list,
        ...
        print(f"Using DeepInfra model: {deepinfra_model}")
        vlm_response = DeepInfraInference(
            prompt=prompt,
            system_prompt=system_prompt,
            image_bytes_list=image_bytes_list,
